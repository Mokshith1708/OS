...
- **Proper Process Termination:** **(PARTIALLY DONE)** Enhanced `_exit` to free a process's PCB and reload the shell. (Still a workaround for single-process memory model).
- **File System:** **(NOT DONE)** Implement a simple, in-memory file system (ramdisk) to manage application binaries and data.
    - Sub-task: Design filesystem structures (`fs.h`). **(DONE)**
    - Sub-task: Create `mkfs` host tool to build ramdisk image. **(DONE)**
    - Sub-task: Load ramdisk into kernel at boot.
    - Sub-task: Implement `open`, `read`, `write`, `close`, `lseek`, `fstat` syscalls.
    - Sub-task: Integrate `sys_exec` with new filesystem.
- **Advanced Scheduler:** **(PARTIALLY DONE)** Enhance the scheduler to handle multiple processes in various states (e.g., Ready, Running, Blocked, Sleeping).
    - Sub-task: Implement `sleep()` syscall.
- **Expanded Syscall Interface:** **(PARTIALLY DONE)** Add new system calls for features like inter-process communication (IPC) and timing.
    - Sub-task: Implement IPC mechanisms.
    - Sub-task: Implement timing syscalls.
- **Device Drivers:** **(NOT DONE)** Develop drivers for specific hardware peripherals.
