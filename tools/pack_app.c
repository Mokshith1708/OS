...
int main(int argc, char **argv) {
    ...
    long sz = ftell(fi);
    fseek(fi, 0, SEEK_SET);
    if (sz <= 0) { fprintf(stderr,"bad input size\n"); fclose(fi); return 1; }
    ...
    hdr.entry_pc   = vt[1];   /* reset handler */
    hdr.flags      = 0;
    ...
    if (fwrite(&hdr, sizeof hdr, 1, fo) != 1) {
        perror("write hdr"); fclose(fi); fclose(fo); return 1;
    }

    char buf[4096];
    size_t n;
    while ((n = fread(buf,1,sizeof buf,fi))>0) {
        if (fwrite(buf,1,n,fo) != n) { perror("write body"); fclose(fi); fclose(fo); return 1; }
    }
    ...
    printf("Packed %s -> %s\n", in, out);
    printf("  ram_size=%ld entry=0x%08x sp=0x%08x\n", sz, hdr.entry_pc, hdr.initial_sp);
